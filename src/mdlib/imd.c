...
/* Send positions from rvec, therefore we need a separate send buffer and conversion */
...
//  This should not be necessary anymore, if we supply a correctly initialized buffer...
//    /* Resize send buffer */
//    srenew(buffer, size);//TODO const, move to init routine
...
#if ((defined WIN32 || defined _WIN32 || defined WIN64 || defined _WIN64) && !defined __CYGWIN__ && !defined __CYGWIN32__)
        /*for whatever reasons, it is called Sleep on windows*/
        Sleep(IMDLOOPWAIT);
#else
        sleep(IMDLOOPWAIT);
#endif
...
{
    /*the lenght of the previously received header tells us the nr of forces we will receive.*/
        ...
        unsigned long Flags)
{
    ...
    /*read environment on master and prepare socket for incoming connections TODO */
    if (MASTER(cr))
    {
